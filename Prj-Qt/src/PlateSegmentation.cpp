...
namespace pr {
    ...
    cv::Rect boxFromCenter(const cv::Point center, int left, int right, int top, int bottom,
                           cv::Size bdSize) {
...
/**
 * @brief PlateSegmentation::templateMatchFinding 模板匹配查找
 * @param respones
 * @param windowsWidth
 * @param candidatePts
 */
            ...
            for (int width = windowsWidth - 5; width < windowsWidth + 5; width++) {
                for (int interval = windowsWidth / 2; interval < windowsWidth; interval++) {
                    ...
                    float loss = ch_prob[cp1_ch] +
                                 engNum_prob[cp2_p0] + engNum_prob[cp3_p1] + engNum_prob[cp4_p2] +
...
/**
 * @brief PlateSegmentation::segmentPlateBySlidingWindows 通过滑动窗口分割
 * @param plateImage 待分割图片
 * @param windowsWidth
 * @param stride
 * @param respones
 */
...
/**
 * @brief PlateSegmentation::segmentPlatePipline 分割车牌
 * @param plateInfo
 * @param stride
 * @param Char_rects
 */
...
/**
 * @brief PlateSegmentation::ExtractRegions 提取区域
 * @param plateInfo
 * @param rects
 */
